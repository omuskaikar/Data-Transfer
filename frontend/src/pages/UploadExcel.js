...
const UploadExcel = () => {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8000/dbmanager/upload-Excel-postgres/",
        formData,
        {
        ...
        {!connected ? (
          ...
          <Grid container spacing={4}>
            <Grid item xs={12} md={6}>
              <Typography
                variant="h5"
                sx={{ fontWeight: "bold", color: "white", textAlign: "center" }} // Bold white text
              >
                Select a Database
              </Typography>
              <Select
